...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#search').on('click', function() {
			...
			$.ajax({
				url: '<?= base_url('staff-ahli/pencarian') ?>',
				type: 'POST',
				data: {
					cari: true,
					query: $('#query').val(),
					...
					for (var i = 0; i < json.result.length; i++) {
						...
						html += '<div class="row" style="margin-top: 1%;">' +
							'<div class="col-md-8 hasil">' +
								'<a href="<?= base_url('staff-ahli/detail-data-') ?>' + nama_kategori + '/' + (nama_kategori == 'tacit' ? json.result[i].knowledge.id_tacit : json.result[i].knowledge.id_explicit) + '"><h3><i class="fa fa-file-o"></i> ' + json.result[i].knowledge.judul + '</h3></a>' +
								'<h5><i>Oleh <a href="<?= base_url('staff-ahli/detail-profile') ?>/' + json.result[i].knowledge.nip +  '">' + json.result[i].knowledge.nama + '</a> pada ' + json.result[i].knowledge.waktu + '</i></h5>' + 
